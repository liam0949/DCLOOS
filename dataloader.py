...
class Data:
    ...
    def __init__(self, args):
        max_seq_lengths = {'oos': 30, 'stackoverflow': 45, 'banking': 55, 'micd-en': 25}
    ...
    def get_examples(self, processor, args, mode='train'):
        ...
        if mode == 'train':
            ...
            for example in ori_examples:
                if (example.label in self.known_label_list) and (example.label is not self.unseen_token):
                    examples.append(example)
                else:
                   if example.label == self.unseen_token:
            ...
            """adding negative set"""
            neg_examples = processor.get_examples(self.data_dir_neg, "neg")
    ...
    def get_loader(self, examples, args, mode='train'):
        ...
        if mode == 'train':
            ...
            # dataloader = DataLoader(datatensor, batch_size=args.train_batch_size, shuffle=True, drop_last=True, num_workers=4, pin_memory=True)
            ...
            # dataloader = DataLoader(datatensor, sampler=sampler, batch_size=args.eval_batch_size, num_workers=2, pin_memory=True)
...
class DatasetProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, mode):
        if mode == 'train':
            ...
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "squad_placeh.tsv")), "neg")
